...
RSpec.describe Feedjira do
  describe ".parse" do
    ...
    context "when there's an available parser" do
      ...
      it "should get the url from a guid where isPermalink=true " do
        feed = Feedjira.parse(sample_permalink_true_feed)
        puts feed.inspect
        expect(feed.entries.first.url).to eq "https://petrolicious.com/articles/here-are-seven-of-the-prettiest-cars-to-ever-claim-the-land-speed-record"
      end
    end
